...
| file | role |
|---|---|
| `run.sh` | **entry point.** Check gate → window and BARs → link cap → GSP → driver stack → report. Runs in your terminal, does not kill the graphical session |
...
`1`–`7` execute in order, `8`–`9` are variants and diagnostics.
