...
# The nvidia-owned DRM card and its PCI IDs, discovered rather than hardcoded.
EGPU_CARD=""; EGPU_VENDOR=""; EGPU_DEVICE=""; EGPU_BDF=""
find_egpu() {
    local c drv
    for c in /sys/class/drm/card[0-9]*; do
        ...
        EGPU_VENDOR=$(cat "$c/device/vendor" 2>/dev/null)
        EGPU_DEVICE=$(cat "$c/device/device" 2>/dev/null)
        EGPU_BDF=$(basename "$(readlink -f "$c/device")")
        return 0
    done
    return 1
}
...
show_state() {
    hdr "Modules"
    local m
    for m in "${NV_MODULES[@]}" egpu_rp_window; do
    ...
    hdr "The card"
    if find_egpu; then
        ok "$EGPU_CARD  $EGPU_BDF"
        lspci -k -s "${EGPU_BDF#*:}" 2>/dev/null | sed 's/^/      /'
    else
...
if [[ $MODE == unload ]]; then
    ...
    find_egpu || warn "no nvidia-owned DRM card - the driver may already be gone"
    ...
    if [[ -n $EGPU_BDF && $(lspci -k -s "${EGPU_BDF#*:}" 2>/dev/null) == *"Kernel driver in use"* ]]; then
        warn "a driver is still bound to $EGPU_BDF"
        lspci -k -s "${EGPU_BDF#*:}" 2>/dev/null | sed 's/^/      /'
    else
...
SUBSYSTEM=="drm", ENV{DEVTYPE}=="drm_minor", ENV{DEVNAME}=="/dev/dri/card[0-9]", SUBSYSTEMS=="pci", ATTRS{vendor}=="$EGPU_VENDOR", ATTRS{device}=="$EGPU_DEVICE", TAG+="mutter-device-ignore"
EOF
